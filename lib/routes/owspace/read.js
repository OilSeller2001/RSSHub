...
const generateFullText = async (item) => {
    const model = Number.parseInt(item.model);
...
const handleItem = (ctx) => async (item) => {
    if (Number.parseInt(item.model) === 4) {
        const result = {
            title: item.title.substring(0, 4) + '年' + Number.parseInt(item.title.substring(4, 6)) + '月' + Number.parseInt(item.title.substring(6, 8)) + '日',
            link: item.html5,
...
module.exports = async (ctx) => {
    const selectedType = ctx.params.type === undefined ? 0 : Number.parseInt(ctx.params.type);
    ...
    const responseItemArray = response.data.datas.filter((item) => Number.parseInt(item.model) !== 5);
